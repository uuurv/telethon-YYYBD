...
async def startup_process():
    ...
    print(f"⌔︙ اهلا بك لقد نصبت تليثون العرب بنجاح 🥁 اذهب الى قناتنا لمعرفة المزيـد ⤵️\n CH : https://t.me/iqthon")
    await verifyLoggerGroup()
...
def start_bot():
  try:
    ...
    iqthon.loop.run_until_complete(iqthon(functions.channels.JoinChannelRequest("L3LL3")))
    iqthon.loop.run_until_complete(iqthon(functions.channels.JoinChannelRequest("Teamtelethon")))
  except Exception as e:
    print(e)
    return False
Checker = start_bot()
if Checker == False:
    print("عذرا لديك حظر مؤقت حاول التنصيب غدا")
    iqthon.disconnect()
    sys.exit()
