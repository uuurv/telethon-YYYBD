...
@iqthon.on(admin_cmd(pattern="تحويل ملكيه ([\s\S]*)"))
async def _(event):
...
async def fetch_info(replied_user, event):
    ...
    caption = "<b>𓍹ⵧⵧⵧⵧⵧⵧⵧⵧᵗᵉˡᵉᵗʰᵒᶰ ᵃʳᵃᵇˢ⁦⁦ⵧⵧⵧⵧⵧⵧⵧⵧ𓍻</b>\n"
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTONAMESTART = gvarstatus("اسم وقتي") == "true"
@iqthon.on(admin_cmd(pattern="usinfo(?:\s|$)([\s\S]*)"))
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if not replied_user:
        return
