        ...
        if query.startswith("فحص") and event.query.user_id == bot.uid:
            buttons = [
                [
                    ...
                    Button.url("DEV", "https://t.me/LLL5L"),
                ]
            ]
            if IQPIC and IQPIC.endswith((".jpg", ".png", "gif", "mp4")):
                result = builder.photo(IQPIC, text=iq, buttons=buttons, link_preview=False)
            elif IQPIC:
                result = builder.document(IQPIC, title="iqthon", text=iq, buttons=buttons, link_preview=False)
            else:
                result = builder.article(title="iqthon", text=iq, buttons=buttons, link_preview=False)
...
async def repo(event):
    if event.fwd_from:
        return
    nline = Config.TG_BOT_USERNAME
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await bot.inline_query(nline, "السورس")
